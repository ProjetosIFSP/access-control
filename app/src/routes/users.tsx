...
export const Route = createFileRoute("/users")({
  ...
  loaderDeps: ({ search: { q, profileIds, page } }) => ({
    q,
    profileIds,
    page,
  }),
  loader: ({ context, deps: { q, profileIds, page } }) =>
    Promise.all([
      context.queryClient.ensureQueryData(
        usersQueryOptions({ q, profileIds, page: page ?? 1 }),
      ),
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between gap-4 pt-2">
                        <p className="text-sm text-muted-foreground">
                          {totalUsers} usuário{totalUsers !== 1 ? "s" : ""} no
                          total
                        </p>
