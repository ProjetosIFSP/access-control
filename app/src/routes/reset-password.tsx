...
function ResetPasswordPage() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (error) {
        toast.error(error.message ?? "Link inválido ou expirado. Solicite um novo.");
        return;
      }
                ...
                <div className="space-y-1.5">
                  ...
                  {field.state.value && (
                    <ul className="mt-1 space-y-0.5">
                      {[
                        { ok: field.state.value.length >= 8, label: "Mínimo 8 caracteres" },
                        { ok: /[A-Z]/.test(field.state.value), label: "Uma letra maiúscula" },
                        { ok: /[0-9]/.test(field.state.value), label: "Um número" },
                      ].map(({ ok, label }) => (
                        <li key={label} className={`flex items-center gap-1.5 text-xs ${ok ? "text-primary" : "text-muted-foreground"}`}>
                          <CheckCircle2 className={`size-3 ${ok ? "text-primary" : "text-muted-foreground/40"}`} />
                          {label}
                        </li>
                      ))}
                    </ul>
