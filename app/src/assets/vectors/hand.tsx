...
const CAPTURE_TOTAL_SECONDS = 30;

// ── HandStrokeOverlay ─────────────────────────────────────────────────────────

/**
 * Renderiza um overlay primary sobre o stroke da mão usando máscara SVG.
 *
 * Estratégia:
 *  - Um <mask> usa o mesmo path da mão com fill branco: só pixels do stroke
 *    ficam visíveis através da máscara (SVG mask: branco = opaco, preto = transparente).
 *  - Um <rect> preenchido com var(--primary) é mascarado por esse mask.
 *  - Um <clipPath> com um <rect> que desce de cima recorta o overlay:
 *      y=0%   → overlay cobre tudo (início da contagem)
 *      y=100% → overlay desaparece completamente (tempo esgotado)
 *  - Base do overlay fixada no fundo; o topo que desce.
...
const today = dayjs();
const isHalloween = today.date() === 31 && today.month() === 9;

// ── FingerButton ──────────────────────────────────────────────────────────────
// Renders a fingerprint button overlaid on the fingertip via foreignObject.
