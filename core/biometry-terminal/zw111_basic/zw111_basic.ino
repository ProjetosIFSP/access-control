...
uint8_t requestTemplateUpload(uint8_t bufferId) {
  ...
  if (packetResult != FINGERPRINT_OK) {
#ifdef DEBUG
    Serial.print(F("[BIO] PS_UpChar(buffer "));
    Serial.print(bufferId);
    Serial.print(F(") falhou ao ler resposta: "));
    Serial.println(packetResult);
#endif
    return packetResult;
  }
...
bool readTemplatePackets(String &templateHex) {
#ifdef DEBUG
  Serial.println(F("[BIO] ACK recebido — lendo data packets..."));
#endif

  uint8_t packetNum = 0;
  while (true) {
    uint8_t dummyPayload[64] = {0};
    Adafruit_Fingerprint_Packet pkt(FINGERPRINT_DATAPACKET, 0, dummyPayload);
    uint8_t pktResult = finger.getStructuredPacket(&pkt, 3000);

    if (pktResult != FINGERPRINT_OK) {
#ifdef DEBUG
      Serial.print(F("[BIO] Erro ao ler packet "));
      Serial.print(packetNum + 1);
      Serial.print(F(": 0x"));
      Serial.println(pktResult, HEX);
#endif
      return false;
    }

    uint16_t dataLen = (pkt.length >= 2) ? (pkt.length - 2) : 0;
    if (templateHex.length() + (dataLen * 2) > 512) {
#ifdef DEBUG
      Serial.println(F("[BIO] ERRO: template maior que 256 bytes."));
#endif
      return false;
    }

    templateHex += bytesToHex(pkt.data, dataLen);
    packetNum++;
...
bool captureTemplateHexFromSensor(String &templateHex) {
  templateHex = "";
  templateHex.reserve(512);

  for (uint8_t attempt = 0; attempt < 3; attempt++) {
#ifdef DEBUG
    Serial.print(F("[BIO] Iniciando UploadTemplate (PS_UpChar) buffer 1"));
    if (attempt > 0) {
    ...
    uint8_t result = requestTemplateUpload(1);
    if (result == FINGERPRINT_OK) {
#ifdef DEBUG
      Serial.println(F("[BIO] Buffer 1 pronto para leitura."));
#endif
      return readTemplatePackets(templateHex);
    }

#ifdef DEBUG
    Serial.print(F("[BIO] getModel(buffer 1) retornou: "));
    Serial.println(result);
#endif
    if (result != TEMPLATE_EMPTY_CODE) {
#ifdef DEBUG
      Serial.println(F("[BIO] Buffer 1 rejeitou o upload."));
#endif
      return false;
    }
    ...
    if (attempt < 2) {
#ifdef DEBUG
      Serial.println(F("[BIO] Buffer 1 ainda vazio, aguardando estabilizar..."));
#endif
      delay(250);
    }
  }
...
#ifdef DEBUG
  Serial.println(F("[BIO] Buffer 1 permaneceu vazio, tentando buffer 2..."));
#endif

  uint8_t fallbackResult = requestTemplateUpload(2);
  if (fallbackResult != FINGERPRINT_OK) {
#ifdef DEBUG
    Serial.print(F("[BIO] getModel(buffer 2) retornou: "));
    Serial.println(fallbackResult);
#endif
    return false;
  }
...
void publishEnrollmentResult(const String &enrollmentIdValue,
  ...
  out["enrollmentId"] = enrollmentIdValue;
  out["userId"] = userIdValue;
  out["finger"] = fingerValue;
  out["status"] = status;
  out["quality"] = quality;
  if (templateHex.length() > 0) out["template"] = templateHex;
  out["deviceSecret"] = DEVICE_SECRET;

  String payloadOut;
  serializeJson(out, payloadOut);
  mqtt.publish(topicEnrollmentResult.c_str(), payloadOut.c_str());
#ifdef DEBUG
  Serial.print(F("[MQTT] Published enrollment-result: "));
...
void publishAccessAttempt(int fingerID) {
  ...
  doc["credentialType"]  = "FINGERPRINT";
...
void onMqttMessage(char* topic, byte* payload, unsigned int length) {
  ...
  if (t == topicEnterEnrollment) {
    ...
    enrollmentExpiresAt = doc["expiresAt"] | 0;
...
void setupFingerprint() {
  ...
  Serial.print(F("[BIO] Nível de segurança (SysPara): ")); Serial.println(finger.security_level);
#endif
      found = true;
      break;
    }
  ...
  switch (enrollmentStep) {
...
#ifdef DEBUG
          Serial.println(F("[BIO] Primeira passagem capturada."));
#endif
        }
      }
      break;
    ...
    case ENROLL_STEP_CREATE_MODEL: {
      ...
      if (enrollmentTemplateHex.length() == 0) {
        String templateHex;
        if (!captureTemplateHexFromSensor(templateHex)) {
          publishEnrollmentResult(enrollmentId, enrollmentUserId, enrollmentFinger, "FAILED", "", 0);
          resetEnrollmentState();
          return;
        }
        enrollmentTemplateHex = templateHex;
      }
