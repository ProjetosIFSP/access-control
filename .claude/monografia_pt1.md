...
Diante disso, este trabalho justifica-se pela necessidade de desenvolver e implementar um sistema de controle de acesso que utilize biometria e RFID e tenha baixo custo para o campus. A proposta visa não apenas aumentar a segurança, mas também automatizar os processos de cadastramento de digitais vigentes no campus, fornecer uma maneira de visualização de dados em tempo real e garantir a flexibilidade de gerenciamento remoto e escalabilidade.
	...
	A metodologia deste trabalho inicia-se com uma revisão bibliográfica sobre microcontroladores e os protocolos de comunicação utilizados por eles, visando identificar estudos relevantes e recentes na área, tais como projetos feitos em outras instituições e a verificação novos produtos deste âmbito no mercado.

Serão analisadas e comparadas as técnicas utilizadas e os tipos de fechaduras eletrônicas inteligentes disponíveis, desta forma, visando uma meta de onde o projeto deve alcançar e quais dores deve sanar. Com base nessa revisão, serão selecionadas algumas técnicas para experimentação inicial, onde será feito um pré-protótipo para testar a comunicação entre os dispositivos e avaliar seu funcionamento. Após essa etapa, será desenvolvida e refinada a implementação do protótipo final. Por fim, serão realizados testes funcionais para validar o desempenho e a eficácia do sistema.
...
A seleção dos trabalhos foi realizada por meio do Google Acadêmico para os trabalhos a fim de garantir uma maior semelhança com este trabalho. Para os produtos disponíveis, foi utilizado o Google tradicional. Os itens selecionados apresentam diferentes implementações e utilizações, contribuindo para uma compreensão geral sobre o controle de acesso e comunicação IoT.

O trabalho de Aniru et al. (2024) propõe uma abordagem de controle de acesso por RFID combinado com uma senha numérica o qual se comunica com um servidor via Wi-Fi para a operação remota de abertura e trancamento das salas. Já o trabalho de Gonçalves (2025), utiliza RFID e biometrica por digital no controle de acesso e se comunica com um servidor *web* PHP para persistência e operação sobre dados.

Em outra perspectiva, temos produtos disponíveis no mercado, tais como a “Smart Fechadura” da Positivo, a qual é um produto que conta com uma fechadura inteligente que possibilita a operação com biometria digital e senha numérica. 
...
O diagrama de blocos apresentado na figura 1 ilustra essa arquitetura, mostrando o fluxo de energia da bateria se dividindo: uma linha direta para o relé (que alimenta a solenoide) e outra passando pelo regulador de tensão para fornecer a energia necessária ao microcontrolador, que por sua vez controla o relé.
...
Contudo, as semelhanças entre os projetos se resumem apenas à camada de *hardware* básica. A análise do trabalho de Aniru et al. (2024) revela limitações significativas considerando o escopo institucional deste TCC. Essas limitações representam as principais dores que este trabalho propõe sanar.

O trabalho de Aniru et al. (2024) depende de uma plataforma de terceiros (Blynk), nisso, todos os logs e comandos de acesso são geridos por esta plataforma. Para uma instituição pública como o IFSP, isso representa um risco de privacidade e segurança de dados, uma vez que não há controle direto de quem pode acessar esses logs ou o que acontece com eles.  
	Ademais, o sistema é totalmente *online*, uma vez que os servidores da Blynk se encontram na rede. E, uma vez que o serviço da plataforma ou o plano gratuito que os autores utilizaram forem descontinuados, a solução proposta torna-se inoperável.

Nesse cenário, o trabalho propõe um servidor MQTT auto-hospedado com Aedes e Node.js e um backend próprio com Fastify e Drizzle ORM com banco de dados PostgreSQL. Isso garante que 100% dos dados de acesso e logs permaneçam sob domínio do próprio campus e permite o sistema operar inteiramente *offline*, dependendo apenas da rede Wi-Fi interna do campus, o que o torna mais seguro e robusto.
...
O plano de negócio de Gonçalves (2025) assemelha com as fechaduras vigentes no IFSP-PEP, uma vez que toda a autenticação e cadastros são feitas no próprio *hardware*. Além disso, assim como nestas fechaduras, a lógica de gerenciamento é realizada localmente. Neste trabalho de Gonçalves (2025), é realizada através de um cartão mestre, responsável para entrar em modo de cadastro ou remoção de identificadores.

No cadastro, o administrador em conjunto de seu cartão mestre, entra em modo de cadastro e, então, aproxima a *tag* RFID do usuário no leitor. O sistema lê o UID do cartão e o salva. Em seguida, o dispositivo inicia o processo de colata e registro da impressão digital, que, uma vez coletada, o dispositivo mostra a confirmação do registro.
...
* **Segurança**: os dispositivos devem ser seguros no sentido que as informações não devem ser possíveis de manipulação e devem resistir à tentativas de falsificação de credenciais.
	...
	A escolha de um sensor de impressão digital deve ser levada a sério, uma vez que existe um problema na interoperabilidade entre diferentes sensores de biometria por impressão digital, uma vez que não existe padrão nos *templates* utilizados pelos fabricantes, logo um valor de leitura num sensor X não será o mesmo num sensor Y, mesmo aplicando a mesma impressão digital. Neste caso, a falta de um formato comum de modelo torna incompatível a comparação entre dados coletados em sensores diferentes.
...
Segundo Kevin Ashton (2009), criador do termo IoT, se o mundo tivesse computadores que soubessem de tudo o que há para saber sobre coisas, usando dados advindos de colatas, sem qualquer interação humana, seria capaz de monitorar e mensurar tudo, reduzindo o desperdício, as perdas e o custo.

Gubbi et al. (2013) definem a IoT como um paradigma que possibilitam a onipresença de uma variedade de coisas ou objetos, isto é, que são capazes de interagir entre si e cooperar com seus vizinhos almejando objetivos comuns. No cenário deste TCC, são as fechaduras eletrônicas que irão interagir com um servidor central a fim de garantir a segurança do controle de acesso e reportar seu estado.
...
Destarte, o intuito desse trabalho é exatamente este: agilizar e facilitar o acesso à salas, facilitar a localização de salas livres, localizar facilmente servidores do IFSP-PEP no campus se estiverem numa sala.
...
Estes estão conectados entre si, para que todos os módulos possam se comunicar, diretamente ou por meio de intermediadores.
...
Para o desenvolvimento dos módulos, foram utilizadas tecnologias e bibliotecas aprendidas durante o curso e vida do discente autor do TCC. Tais como Node.js com o *framework* Fastify para as funções de *back-end*, que se comunicam com um banco de dados PostgreSQL via Drizzle ORM.
...
Já para o *front-end*, foi utilizado React, uma biblioteca moderna de desenvolvimento *web* baseada em JavaScript. Esta torna capaz o desenvolvimento ágil e modular de uma SPA (Aplicação de Página Única), que é uma aplicação *web* a qual o usuário interage apenas com um arquivo HTML único, o qual a troca de informação é feita inteiramente pelo JavaScript, ou neste caso pelo TypeScript, um *superset* de JavaScript que permite a tipagem, visando escalabilidade e manutenibilidade.
...
Por fim, para se conectar ao *back-end*, foi utilizado Tanstack Query para fazer as chamadas aos *endpoints* disponíveis. Assim essa biblioteca as faz monitorando, mantendo em cache e sincronizando os dados com a API.
...
Adjunto, para que estes dados sejam persistidos, utiliza o Drizzle ORM, que como o nome sugere, é um ORM (Object Relational Mapping) que traduz o código implementado em chamadas SQL (Structured Query Language). 

O banco de dados utilizado, por sua vez, é o PostgreSQL, um sistema gerenciador de banco de dados (SGBD) relacional, ou seja, que as informações podem estar interligadas umas às outras.
...
Tanto a API REST quanto o servidor MQTT, eles fornecem URLs (Uniform Resource Locator) para acesso, seja para *endpoints*, ou para canais de assinatura. Estas URLs podem ser especificadas o que se espera de entrada e saída.

Pensando nisso, foi desenvolvido com uma ferramenta chamada Swagger, uma página *web* que descreve essas rotas de URL e é possível visualizar suas entradas e saídas, bem como testá-las. Esse desenvolvimento visa a qualidade e consistência no desenvolvimento e utilização de APIs.

## **4.2 Desenvolvimento parcial** {#4.2-desenvolvimento-parcial}

Nesta seção são descritos os módulos desenvolvidos até o momento, que abrangem mais a parte da comunicação de dispositivos IoT no geral com o servidor e a parte de visualização e manutenção dos dados.

Até o momento, foram desenvolvidos alguns *endpoints* da API e iniciado o consumo dessas informações pela aplicação *web* em React. Foram desenvolvidos rotas de usuários do sistema, salas, blocos de salas, controladores de portas e de tentativas de acesso.

Para isso, também foi desenvolvido, via ORM, a base de dados necessária para o funcionamento do sistema.
...
Neste, foi iniciado uma aplicação React com Vite, Tailwind CSS, Shadcn, Tanstack Router e Tanstack Query capaz de mostrar informações gerais e básicas sobre as salas cadastradas.

#####  **Figura 15\. Página React.** {#figura-15.-página-react.}

![][image16]

Fonte: Autor

Esta tela ainda se encontra em fase inicial, foi criada apenas para teste de busca de informações na base de dados.
...
Durante a execução, o sistema demonstrou capacidade de consumir dados previamente inseridos no banco de dados por *seeds*, tal como foi possível utilizar os endpoints, tanto em requisições cURL, na plataforma *web* ou até mesmo na especificação do Swagger.

Foi testado apenas de maneira básica a troca de informações pelos dispositivos IoT com o servidor, mas com o sucesso obtido, torna-se promissor o resultado final almejado.

Para prosseguir no desenvolvimento do objetivo final deste trabalho de conclusão de curso, será necessário desenvolver outras *endpoints* para a API, qualquer outro canal de assinatura MQTT necessário, implementar outras telas *web* e por fim e mais importante, implementar o protótipo final da fechadura eletrônica, se preocupando com interferências na rede e energia.

Para a fechadura, será necessário também implementar e testar o cadastro e utilização das credenciais de impressão digital ou RFID, fator chave para a viabilidade do projeto.
